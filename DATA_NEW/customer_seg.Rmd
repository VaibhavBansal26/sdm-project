...
custData[is.na(custData)] <- lapply(custData[is.na(custData)], function(x) mean(x, na.rm = TRUE))

# Encode categorical variables (e.g., Country, Gender, PaymentMethod)
custData$Country <- as.numeric(factor(custData$Country))
custData$Gender <- as.numeric(factor(custData$Gender))
custData$PaymentMethod <- as.numeric(factor(custData$PaymentMethod))

features <- custData %>%
  select(Age, UnitPrice, Quantity, Country, Gender, PaymentMethod)
...
fviz_cluster(kmeans_result, data = features_scaled, geom = "point", ellipse.type = "norm") +
